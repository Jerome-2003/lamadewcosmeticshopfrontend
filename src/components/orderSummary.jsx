...
export default function OrderSummary({ cart = [], onOrderProcessed }) {
  ...
  //   // Validating against the full field constraint present in site.js
...
const processOrder = async () => {
  ...
  try {
    // Make the request directly to your Node.js server port
    const response = await fetch('http://localhost:3001/api/orders', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        // Include authorization token saved on browser storage during user login
    ...
    alert('Order successfully saved into the backend database!');
  } catch (error) {
  ...
  return (
    <div className="lg:col-span-1">
      <div className="rounded-[2rem] border p-6 glass-card sticky top-24 bg-white/80 shadow-sm">
        ...
        <button
          type="button"
          className="w-full bg-[#705338] text-white rounded-full px-6 py-4 font-semibold mt-6 transition hover:opacity-90 active:scale-98 shadow-md"
          onClick={processOrder}
        >
          Process Order
        </button>
